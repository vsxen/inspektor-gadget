...
static __always_inline int __trace_tcp_drop(void *ctx, struct sock *sk, struct sk_buff *skb, int reason)
{
	...
	u_int8_t *tcphdr = (u_int8_t *)(BPF_CORE_READ(skb, head) + BPF_CORE_READ(skb, transport_header));
	...
	bpf_probe_read_kernel(&event.tcpflags, sizeof(event.tcpflags), tcphdr+13);
