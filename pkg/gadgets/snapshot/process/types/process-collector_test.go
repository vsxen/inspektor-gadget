...
func TestCreateTree(t *testing.T) {
	...
	expectedPid := 42
	if tree.process.Pid != expectedPid {
		t.Fatalf("root PID is wrong, expected %d got %d", expectedPid, tree.process.Pid)
	}
...
parentLoop:
	for _, node := range tree.children {
		for _, expectedChildrenPID := range expectedChildrenPIDs {
			if node.process.Pid == expectedChildrenPID {
				continue parentLoop
			}
		}
		t.Fatalf("node PID is wrong, expected one from %v got %d", expectedChildrenPIDs, node.process.Pid)
	}

	var specificNode *processTree
	searchingPid := 43
	for _, node := range tree.children {
		if node.process.Pid == searchingPid {
			specificNode = node
	...
	pid := specificNode.children[0].process.Pid
	expectedChildPid := 46
