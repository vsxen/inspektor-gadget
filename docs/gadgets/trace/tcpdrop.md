...
NODE             NAMESPACE  POD    CONTAINER  PID      COMM  IP SADDR       DADDR    SPORT DPORT STATE        TCPFLAGS  REASON
```
...
```
NODE             NAMESPACE  POD    CONTAINER  PID      COMM  IP SADDR       DADDR    SPORT DPORT STATE        TCPFLAGS  REASON
minikube-docker  default    shell  shell      2274721  curl  4  10.244.0.4  1.1.1.1  35478 80    ESTABLISHED  PSH|ACK   QDISC_DROP
```
...
- In terminal 2:
```
$ docker run -ti --rm --name=netem busybox
```

- In terminal 3, use the network emulator (netem) qdisc to ramdomly drop 25% of packets:
```
$ export PID=$(docker inspect netem|jq '.[0].State.Pid')
$ sudo nsenter -t $PID -n tc qdisc del dev eth0 root netem
$ sudo nsenter -t $PID -n tc qdisc add dev eth0 root netem drop 25%
```

- Back in terminal 2, download a web page. It will likely succeed despite the dropped packets because TCP is a reliable protocol:
```
$ wget 1.1.1.1
```

- The results in terminal 1 will show that some packets are dropped by the qdisc:
```
$ sudo ig trace tcpdrop -r docker
CONTAINER  PID        COMM       IP SADDR       DADDR    SPORT DPORT STATE        TCPFLAGS  REASON
netem      3807012    wget       4  172.17.0.2  1.1.1.1  54070 80    ESTABLISHED  PSH|ACK   QDISC_DROP
           0          swapper/2  4  172.17.0.2  1.1.1.1  54070 80    ESTABLISHED  PSH|ACK   QDISC_DROP
```
